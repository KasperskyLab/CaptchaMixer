...
/// <summary>
/// Creates a grid of straight lines along Y axis.
/// </summary>
public class LinesGridY : PatternCreator
{
	/// <summary>
	...
	public LinesGridY(ValueProvider<float> spacing)
	...
	protected override VectorPath CreatePattern(RectangleF area, int maxCount)
	{
		...
		for (var x = area.Left; x < area.Right && instructions.Count / 2 < maxCount; x += 1 + spacing)
		{
			instructions.Add(new MoveToInstruction(x + OffsetFromX, area.Top + OffsetFromY));
			instructions.Add(new LineToInstruction(x + OffsetToX, area.Bottom + OffsetToY));
		}
